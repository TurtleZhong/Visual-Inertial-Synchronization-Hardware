  ...
  while(ros::ok())
  {
    try
    {
      if (ser.isOpen())
      {
		...
		ros::Time measurement_time(ts / 1000000, (ts % 1000000) *1000);  // sec, nsec
 		  ...
 		  trigger_time_msg.header.frame_id = frame_id;
  	   	  ...
  	   	  lastTriggerCounter = triggerCounter;
                ...
                if (broadcast_tf)
                {
                  ...
                  transformStamped.transform.rotation.x = differential_rotation.x();
                  transformStamped.transform.rotation.y = differential_rotation.y();
                  transformStamped.transform.rotation.z = differential_rotation.z();
                  transformStamped.transform.rotation.w = differential_rotation.w();
